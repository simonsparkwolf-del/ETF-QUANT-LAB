"""
Baseline Long-Short — Alpha signal screening (in-sample).

Each alpha in ALPHA_IDS is backtested via BaselineStrategy + BaselineRisk.

Outputs:  backtests/baseline/in_sample/alpha/outputs/
  comparison.md
  summary.json
  best_alpha_{id}/
"""
...
def main() -> None:
    ...
    outputs = paths["ROOT"] / "backtests" / "baseline" / "in_sample" / "alpha" / "outputs"
    ...
    for aid in ALPHA_IDS:
        ...
        metrics = _run_one(
            ...
            run_name=f"is_baseline_alpha_{aid}",
            db_path=db_path,
            save_path=outputs,
    ...
    best_metrics = _run_one(
        ...
        run_name=f"is_baseline_best_alpha_{best_aid}",
        db_path=db_path,
        save_path=best_save,
        save_artifacts=True,
    )
