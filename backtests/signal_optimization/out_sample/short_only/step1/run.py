"""
Step 1 — Single signal quality pre-screening (short-only mode).

Backtests six signals through SiganlOptimizationStrategy (mode="short"):
...
Short sizing: softmax(-score) — worst-ranked ETFs get highest short weight.
...
Outputs written to  backtests/signal_optimization/short_only/step1/outputs/:
...
def main() -> None:
    ...
    outputs = root / "backtests" / "signal_optimization" / "out_sample"/ "short_only" / "step1" / "outputs"
    ...
    for sid in range(0, 6):
        ...
        metrics = _run_one(
            signal,
            run_name=f"short_step1_sig{sid}",
            db_path=db_path,
            save_path=outputs,
    ...
    # ── Phase 2: select best ML signal by Sharpe ─────────────────────────
    ...
    best_metrics = _run_one(
        signal_best,
        run_name=f"short_step1_best_signal_{best_id}",
        db_path=db_path,
        save_path=best_save,
        save_artifacts=True,
    )
