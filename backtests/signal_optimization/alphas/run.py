"""
Alpha screening — same alpha id set as signal_1 LightGBM features.

Each ``alpha_id`` in ``ALPHA_IDS`` is backtested via ``SiganlOptimizationStrategy``;
scores read from ``QuoteTerminal.bars()`` columns ``alpha_{id}``.

Outputs:  backtests/signal_optimization/alphas/outputs/
  alpha_comparison.md
  alpha_summary.json
  best_alpha_{id}/
"""
...
from QuantLab.backtest.signal.alpha_test import ALPHA_IDS, AlphaBacktestSignal
...
def _run_one(
    ...
    *,
    ...
    strategy = SiganlOptimizationStrategy(run_name, long_cost=0.0, base_slippage=0.0)
...
def _build_alpha_report(all_metrics: dict[int, dict], best_aid: int) -> str:
    lines = [
...
def main() -> None:
    ...
    outputs = root / "backtests" / "signal_optimization" / "alphas" / "outputs"
    ...
    for aid in ALPHA_IDS:
        ...
        print(f"{'=' * 60}")
        signal = AlphaBacktestSignal(aid, START_DATE, END_DATE)
        metrics = _run_one(
            signal,
            run_name=f"alpha_screen_{aid}",
            db_path=db_path,
            save_path=outputs,
    ...
    (outputs / "alpha_comparison.md").write_text(
        _build_alpha_report(all_metrics, best_aid), encoding="utf-8"
    )
    ...
    best_metrics = _run_one(
        AlphaBacktestSignal(best_aid, START_DATE, END_DATE),
        run_name=f"alpha_best_{best_aid}",
        db_path=db_path,
        save_path=best_save,
        save_artifacts=True,
    )
