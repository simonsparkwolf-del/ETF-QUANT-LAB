...
def _run_sp(
    ...
    *,
    ...
    strategy = SiganlOptimizationStrategy(run_name, mode="short")
...
def _make_objective(db_path: Path, scratch: Path):
    def objective(trial: optuna.Trial) -> float:
        raw = {aid: trial.suggest_float(f"u{aid}", 0.0, 1.0) for aid in SP_CANDIDATES}
...
def main() -> None:
    ...
    raw_best   = {aid: best_trial.params[f"u{aid}"] for aid in SP_CANDIDATES}
    ...
    with tqdm(total=len(eval_runs), desc="Evaluating", unit="run") as pbar:
        for label, signal, window, start, end, save_arts in eval_runs:
            ...
            m = _run_sp(signal, start, end, db_path, save_dir,
