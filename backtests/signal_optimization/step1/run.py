...
LAMBDA_TURNOVER = 0.1  # §3 objective: L = Sharpe - λ * Turnover_ann
...
def _objective(m: dict) -> float:
    #  return float(m.get("sharpe", 0.0)) - LAMBDA_TURNOVER * float(m.get("turnover_annualized", 0.0))
...
def _build_report(
    ...
    lines = [
        ...
        f"λ (turnover penalty): {LAMBDA_TURNOVER}",
        "",
        "## Comparison Table",
        "",
    ...
    for sid in sorted(all_metrics):
        ...
        lines.append(
            ...
            f"{_pct(max_dd)} | {_pct(to_ann)} | {_f2(obj)} | {d_sharpe} |"
        )

    lines += [
        "",
        f"> ★ best ML signal by objective L = Sharpe − {LAMBDA_TURNOVER} × Turnover_ann",
        "",
        "## Verdict",
        "",
        f"Best ML signal: **{_SIGNAL_LABELS[best_id]}** (Signal {best_id})",
        f"- Sharpe: {_f2(float(all_metrics[best_id].get('sharpe', 0.0)))}",
        f"- Objective L: {_f2(_objective(all_metrics[best_id]))}",
        "",
...
def main() -> None:
    ...
    outputs = root / "backtests" / "signal_optimization" / "step1" / "outputs"
    ...
    print(f"\n>>> Best ML signal: {best_id} ({best_label}), L={_objective(all_metrics[best_id]):.3f}")
