...
Step 1 screening results → `00_step1_screening.md`
...
├── 00 screening/               Step 1 — single-signal LP/SP sweep
│   ├── in_sample/
│   │   ├── long_only/
│   │   │   ├── alphas/run.py   IS LP sweep — all alpha_ids
│   │   │   └── step1/run.py    IS LP sweep — ML signals 1–5
│   │   └── short_only/
│   │       ├── alphas/run.py   IS SP sweep — all alpha_ids
│   │       └── step1/run.py    IS SP sweep — ML signals 1–5
│   └── out_sample/
│       ├── long_only/
│       │   ├── alphas/run.py   OOS LP sweep — all alpha_ids
│       │   └── step1/run.py    OOS LP sweep — ML signals 1–5
│       └── short_only/
│           ├── alphas/run.py   OOS SP sweep — all alpha_ids
│           └── step1/run.py    OOS SP sweep — ML signals 1–5
└── 01 blend/                   Step 2 — signal blend (Bayesian opt)
    ├── long power/             LP blend optimisation — see signal_opt/01_step2_lp_blend.md
    └── short power/            SP blend optimisation (planned)
```
